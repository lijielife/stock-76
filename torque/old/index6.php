...
$texto .="<ul class=\"sousmenu\">
			...
			<li class=\"sousmenu\" ><a href=\"./option\" >Option de torque</a></li>
			</ul>
			<br />
			</div>";
...
for ($i=17; $i > 10 ; $i--) {
	...
	if($presq->dent==$i){
	$texto .="<span class=\"lesdents\" style=\"width:40px;display:block;padding-right:3px;padding-left:3px;margin:1px;color:#".$presq->txtcolor.";border: 2px solid #".$presq->txtcolor.";background-color:#".$presq->bgcolor."\">$presq->dent</span><span style='font-size:12px'>$presq->torque</span></td>\n";
	$prescrip = $requiqui->row();
	}
	else{
	$texto .="<span class=\"lesdents\" style=\"width:40px;display:block;padding-right:3px;padding-left:3px;margin:1px;color:#eee;background-color:#fff;border:2px solid #eee;\">$i</span><span style='font-size:12px'>&nbsp;</span></td>\n";
	}
}
...
for ($i=31; $i < 38 ; $i++) {
  ...
  $presq = $rekiki->row();
	...
	$texto .="<span class=\"lesdents\" style=\"width:40px;display:block;padding-right:3px;padding-left:3px;margin:1px;color:#".$presq->txtcolor.";background-color:#".$presq->bgcolor.";border:2px solid #".$presq->txtcolor.";\">$i</span><span style='font-size:12px'>$presq->torque</span></td>\n";
	}
}
$texto .="<th class=\"selecttorque\">&nbsp;</th></tr>";
...
// fin resume
	...
	$lanature=$torq_cat->nature;
		...
		while($torq = $requi->row()){
			...
			$textito .="<td class=\"$laclasse\" $rajdedent><a class='$laclasse' onMouseOver=\"window.status='$torq->dent:$lanature &agrave; $torq->valeur'; return true;\" href='javascript:choostorq($torq->dent,\"".$torq->valeur."\",$lidcateg,\"$lacolorfond\",\"$lacolortext\",\"$lanature\",\"".$torq->produit_id."\");'>$torq->valeur</a></td>\n";
...
while($torq_cat = $req->row()) {
	...
	$lanature=$torq_cat->nature;
		...
		while($torq = $requi->row()){
			...
			if($torq->dent=="11"){$rajdedent="style=\"border-right: 3px solid #000;\"";}else{$rajdedent="";}
			$textito .="<td class=\"$laclasse\" $rajdedent><a class='$laclasse' href='javascript:choostorq($torq->dent,\"".$torq->valeur."\",$lidcateg,\"$lacolorfond\",\"$lacolortext\",\"$lanature\",\"".$torq->produit_id."\");'>$torq->valeur</a></td>\n";
...
//fin torq mand	
	...
	case "voir";
	$dossier = $_POST['dossier'];
	if($_GET['dossier']!=''){$dossier = $_GET['dossier'];}
// resume prescription
if($dossier!=""){
$requiqui = new db();
$rekiki = new db();
...
for ($i=17; $i > 10 ; $i--) {
	...
	if($i==11){$texto .="<td  class=\"dentcadran1\" >";}else{$texto .="<td class=\"dentcadransup\"";}
	if($presq->dent==$i){
	$texto .="<span class=\"lesdents\" style=\"width:40px;display:block;padding-right:3px;padding-left:3px;margin:2px;color:#".$presq->txtcolor.";border: 2px solid #".$presq->txtcolor.";background-color:#".$presq->bgcolor."\">$presq->dent</span><span style='font-size:12px'>$presq->torque</span></td>\n";
	$prescrip = $requiqui->row();
	}
	else{
	$texto .="<span class=\"lesdents\" style=\"width:40px;display:block;padding-right:3px;padding-left:3px;margin:2px;color:#eee;background-color:#fff;border:2px solid #eee;\">$i</span><span style='font-size:12px'>&nbsp;</span></td>\n";
	}
}
...
for ($i=31; $i < 38 ; $i++) {
	...
	$texto .="<span class=\"lesdents\" style=\"width:40px;display:block;padding-right:3px;padding-left:3px;margin:2px;color:#".$presq->txtcolor.";background-color:#".$presq->bgcolor.";border:2px solid #".$presq->txtcolor.";\">$i</span><span style='font-size:12px'>&nbsp;</span></td>\n";
	}
}
$texto .="<th class=\"selecttorque\">&nbsp;</th>";
	$texto .="</tr></table>";
//historique
	...
	list ($ladateuxe, $lheure) = split (" ", $prescr->date);
		list ($year, $month, $day) = split ("-", $ladateuxe);
...
for ($i=17; $i > 10 ; $i--) {
  ...
  $texto .="<span class=\"lesdents\" style=\"width:40px;display:block;padding-right:3px;padding-left:3px;margin:2px;color:#".$prescr->txtcolor.";border: 2px solid #".$prescr->txtcolor.";background-color:#".$prescr->bgcolor."\">$prescr->dent</span><span style='font-size:12px'>$prescr->torque</span>";
	$texto .="</td>\n";
	$prescr = $reqpre->row();
}
...
for ($i=21; $i < 28 ; $i++) {
	if($prescr->torque==""){$prescr->torque= "&nbsp;";}
	$texto .="<td class=\"dentcadransup\" >
  <span class=\"lesdents\" style=\"width:40px;display:block;padding-right:3px;padding-left:3px;margin:2px;color:#".$prescr->txtcolor.";border: 2px solid #".$prescr->txtcolor.";background-color:#".$prescr->bgcolor."\">$prescr->dent</span><span style='font-size:12px'>$prescr->torque</span>";
	$texto .="</td>\n";
	$prescr = $reqpre->row();
}
$texto .='<th class="selecttorque">&nbsp;</th>
