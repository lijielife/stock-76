...
$req = new db();
	...
	$texto .='<form name="prescription" method="POST" action="./save">
		N&deg; dossier <input type="text" class="actif"  readonly name="dossier" id="dossier" value="'.$_POST['dossier'].'" >
...
// fin resume
	...
	$req->findquery("SELECT * FROM torq_categ ORDER BY nature DESC, categ ASC");
	$texto .= '';
	...
	$lanature=$torq_cat->nature;
		...
		while($torq = $requi->row()){
			...
			$textito .="<td class=\"$laclasse\" $rajdedent><a class='$laclasse' href='javascript:choostorq($torq->dent,\"".$torq->valeur."\",$lidcateg,\"$lacolorfond\",\"$lacolortext\",\"$lanature\");'>$torq->valeur</a></td>\n";

      $all3="choostorq($torq->dent,\"".$torq->valeur."\",$lidcateg,\"$lacolorfond\",\"$lacolortext\",\"$lanature\");";
