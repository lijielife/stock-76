...
switch($_GET['action']){
	...
	$titid = $categ->id_titre;
		...
		list ($year, $month, $day) = split ("-", $leproduit->date_commande);
	...
	<input type=\"hidden\" name=\"oldid\" value=\"".$_GET['oldid']."\">
	<input type=\"hidden\" name=\"oldfourni\" value=\"".$_GET['oldfourni']."\">
	<input type=\"hidden\" name=\"oldaction\" value=\"".$_GET['oldact']."\">
		<input type=\"hidden\" name=\"oldordre\" value=\"".$_GET['oldordre']."\">
...
<input style=\"float:right\" type=\"button\" value=\"commander/imprimer\" onclick=\"this.form.action='./commanderimprimer';this.form.submit()\">
  <table cellspacing=\"0\" style=\"clear:both\" >
	...
	while($produit = $reqpre->row()){
		...
		<td><input name=\"action\" type=\"hidden\" value=\"commander\"><input name=\"lid\" type=\"hidden\" value=\"$produit->id_produit\">
		 <input type=\"submit\" name=\"submitbutton\" value=\"command&eacute;\" id=\"submitbutton\"></td>";
		}
		else{
	...
	while($produit = $reqpre->row()){
		if($produit!=""){$texto .= "<tr class=\"stock\" align='center'>
		<td  colspan=\"13\"> --------- produit non utilise ---------	</td></tr>";
	}
		$tot = floatval($produit->stock) * floatval($produit->prix);
		$totaligne = number_format($tot, 2, ',', '');
		list ($year, $month, $day) = split ("-", $produit->date_commande);
